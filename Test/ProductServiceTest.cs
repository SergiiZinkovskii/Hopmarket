...
using Xunit;
    ...
    namespace Market.Services.Tests
    {
        ...
        public async Task Create_ValidProduct_ReturnsBaseResponseWithCreatedProduct(
            ProductViewModel model, List<byte[]> imageDataList)
        {
            // Arrange
            var product = _fixture.Build<Product>()
                .With(p => p.Id, 1)
                .Create();
            _productRepository.Create(Arg.Any<Product>()).Returns(Task.FromResult(product));
        ...
        public async Task Create_WithException_ReturnsBaseResponseWithInternalServerError(
            ProductViewModel model, List<byte[]> imageDataList, Exception exception)
        {
            // Arrange
            var productRepository = new ProductRepository(); 
            var productService = new ProductService(productRepository, _commentRepository);

            // Act
            var result = await productService.Create(model, imageDataList);
